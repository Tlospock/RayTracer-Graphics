 ...
 */
float*
glmWeldVectors(float* vectors, uint32_t* numvectors, float epsilon)
{
  float* copies;
  uint32_t   copied;
  uint32_t   i, j;
...
glmFirstPass(GLMmodel* model, FILE* file)
{
  ...
  group = model->groups;
  while(group) {
    group->triangles = (uint32_t*)malloc(sizeof(uint32_t) * group->numtriangles);
    group->numtriangles = 0;
    group = group->next;
  }
}
 ...
 */
void
glmWriteOBJ(GLMmodel* model, char* filename, uint32_t mode)
{
  uint32_t    i;
  FILE*     file;
  GLMgroup* group;
 ...
 */
void
glmDraw(GLMmodel* model, uint32_t mode)
{
  static uint32_t i;
...
glmWeld(GLMmodel* model, float epsilon)
{
  ...
  for (i = 0; i < model->numtriangles; i++) {
    T(i).vindices[0] = (uint32_t)vectors[3 * T(i).vindices[0] + 0];
    T(i).vindices[1] = (uint32_t)vectors[3 * T(i).vindices[1] + 0];
    T(i).vindices[2] = (uint32_t)vectors[3 * T(i).vindices[2] + 0];
  }
  ...
  for (i = 0; i < model->numtriangles; i++) {
    T(i).nindices[0] = (uint32_t)vectors[3 * T(i).nindices[0] + 0];
    T(i).nindices[1] = (uint32_t)vectors[3 * T(i).nindices[1] + 0];
    T(i).nindices[2] = (uint32_t)vectors[3 * T(i).nindices[2] + 0];
  }
  ...
  for (i = 0; i < model->numtriangles; i++) {
    for (j = 0; j < 3; j++) {
      T(i).tindices[j] = (uint32_t)vectors[3 * T(i).tindices[j] + 0];
    }
  }
